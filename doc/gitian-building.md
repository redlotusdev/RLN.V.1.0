...
*Setup instructions for a Gitian build of Lotus Core using a Debian VM or physical system.*
...
- [Building Lotus Core](#building-lotus-core)
...
Clone the git repositories for Lotus Core and Gitian.

```bash
git clone https://github.com/devrandom/gitian-builder.git
git clone https://github.com/lotuscoin/lotuscoin
```
...
in the Lotus Core repository under 'Fetch and build inputs' to install sources which require
...
To build Lotus Core (for Linux, OS X and Windows) just follow the steps under 'perform
Gitian builds' in [doc/release-process.md](release-process.md#perform-gitian-builds) in the Lotus Core repository.
...
```bash
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/lotus/.git/
    remote: Counting objects: 57959, done.
    remote: Total 57959 (delta 0), reused 0 (delta 0), pack-reused 57958
    Receiving objects: 100% (57959/57959), 53.76 MiB | 484.00 KiB/s, done.
    Resolving deltas: 100% (41590/41590), done.
    From https://github.com/lotuscoin/lotuscoin
...
For example:
```bash
URL=https://github.com/crowning-/lotus.git
COMMIT=b616fb8ef0d49a919b72b0388b091aaec5849b96
./bin/gbuild --commit lotus=${COMMIT} --url lotus=${URL} ../lotus/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit lotus=${COMMIT} --url lotus=${URL} ../lotus/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit lotus=${COMMIT} --url lotus=${URL} ../lotus/contrib/gitian-descriptors/gitian-osx.yml
```
...
and the lotus git repository with the desired tag must both be available locally, and then gbuild must be
...
LXC_ARCH=amd64 LXC_SUITE=precise on-target -u root \
  ...
  $( sed -ne '/^packages:/,/[^-] .*/ {/^- .*/{s/"//g;s/- //;p}}' ../lotus/contrib/gitian-descriptors/*|sort|uniq )
...
BTCPATH=/some/root/path/lotuscoin.git
SIGPATH=/some/root/path/lotuscoin-detached-sigs.git

./bin/gbuild --url lotus=${BTCPATH},signature=${SIGPATH} ../lotus/contrib/gitian-descriptors/gitian-win-signer.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/lotus-linux-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/lotus-win-build.assert
    gpg --detach-sign ${VERSION}-osx-unsigned/${SIGNER}/lotus-osx-build.assert
```
...
[lotus/gitian.sigs](https://github.com/lotuscoin/gitian.sigs/) repository, or if that's not possible to create a pull
request.
