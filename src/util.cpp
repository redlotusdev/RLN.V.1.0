...
const char * const BITCOIN_PID_FILENAME = "lotusd.pid";
...
bool LogAcceptCategory(const char* category)
{
    if (category != NULL)
    {
        ...
        if (ptrCategory.get() == NULL)
        {
            ...
            // "lotus" is a composite category enabling all Lotus-related debug output
            if(ptrCategory->count(string("lotus"))) {
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\LotusCore
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\LotusCore
    // Mac: ~/Library/Application Support/LotusCore
    // Unix: ~/.lotuscore
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "LotusCore";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/LotusCore";
#else
    // Unix
    return pathRet / ".lotuscore";
#endif
#endif
}
                    ...
                    map<string, vector<string> >& mapMultiSettingsRet)
{
    ...
    if (!streamConfig.good()){
        // Create empty lotus.conf if it does not excist
        ...
        if (configFile != NULL) {
        	...
        	LogPrintf("lotus.conf file not found or can't be created\n");
    ...
    {
        // Don't overwrite existing settings so command line settings override lotus.conf
