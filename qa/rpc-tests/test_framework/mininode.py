# mininode.py - Lotus P2P network half-a-node
#
...
#
# NodeConn: an object which manages p2p connectivity to a lotus node
...
def lotushash(s):
    return lotus_hash.getPoWHash(s)
...
class CBlockHeader(object):
    ...
    def calc_sha256(self):
        if self.sha256 is None:
            ...
            r += struct.pack("<I", self.nBits)
            r += struct.pack("<I", self.nNonce)
            self.sha256 = uint256_from_str(lotushash(r))
            self.hash = encode(lotushash(r)[::-1], 'hex_codec').decode('ascii')
...
class msg_headers(object):
    ...
    def deserialize(self, f):
        # comment in lotusd indicates these should be deserialized as blocks
...
class NodeConn(asyncore.dispatcher):
    ...
    def __init__(self, dstaddr, dstport, rpc, callback, net="regtest", services=1):
        ...
        print 'MiniNode: Connecting to Lotus Node IP # ' + dstaddr + ':' \
            + str(dstport)
